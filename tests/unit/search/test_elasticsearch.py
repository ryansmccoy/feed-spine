...
pytestmark = pytest.mark.skipif(not HAS_ELASTICSEARCH, reason="elasticsearch package not installed")
...
class TestElasticsearchSearchInitialization:
    ...
    async def test_creates_index_on_initialize(self, mock_es_client: AsyncMock) -> None:
        ...
        with patch("feedspine.search.elasticsearch.AsyncElasticsearch", return_value=mock_es_client):
