...
from feedspine.storage.backends.duckdb import DuckDBStorage  # noqa: E402
...
class TestDuckDBStorageLifecycle:
    ...
    async def test_creates_tables_on_initialize(self, tmp_path: Path) -> None:
        ...
        result = storage._conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
...
class TestDuckDBStorageRecordSightingTracking:
    ...
    async def test_record_sighting_on_existing_updates_fields(self, memory_storage: DuckDBStorage) -> None:
    ...
    async def test_record_sighting_on_existing_preserves_first_seen(self, memory_storage: DuckDBStorage) -> None:
    ...
    async def test_record_sighting_on_existing_nonexistent_returns_none(self, memory_storage: DuckDBStorage) -> None:
    ...
    async def test_stored_record_has_sighting_fields(self, memory_storage: DuckDBStorage) -> None:
    ...
    async def test_sighting_tracking_in_sql_queries(self, memory_storage: DuckDBStorage) -> None:
...
class TestDuckDBAnalytics:
    ...
    async def test_raw_sql_query(self, memory_storage: DuckDBStorage) -> None:
        ...
        for i in range(5):
            ...
            record = record.model_copy(update={"content": {"company": "Acme Corp", "revenue": (i + 1) * 1000}})
            await memory_storage.store(record)

        # Execute analytics query (layer values are lowercase)
        results = await memory_storage.execute_sql("SELECT COUNT(*) as cnt FROM records WHERE layer = 'gold'")
    ...
    async def test_json_content_query(self, memory_storage: DuckDBStorage) -> None:
        ...
        await memory_storage.store(
            make_record("json-1").model_copy(update={"content": {"company": "Apple", "sector": "Tech"}})
        )
        await memory_storage.store(
            make_record("json-2").model_copy(update={"content": {"company": "Microsoft", "sector": "Tech"}})
        )
        await memory_storage.store(
            make_record("json-3").model_copy(update={"content": {"company": "Exxon", "sector": "Energy"}})
        )
...
class TestDuckDBParquet:
    ...
    async def test_export_parquet_with_query(self, memory_storage: DuckDBStorage, tmp_path: Path) -> None:
    ...
    async def test_export_empty_returns_zero(self, memory_storage: DuckDBStorage, tmp_path: Path) -> None:
