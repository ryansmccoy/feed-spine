...
class TestDuckDBStorageBatch:
    ...
    async def test_store_batch_skip_duplicates(self, storage: DuckDBStorage) -> None:
        ...
        await storage.store_batch(records, on_conflict="skip")
    ...
    @pytest.mark.slow
    @pytest.mark.timeout(60)  # Large batch test needs more time
    ...
    @pytest.mark.slow
    @pytest.mark.timeout(60)  # Chunking test with large dataset needs more time
    ...
    async def test_delete_batch_basic(self, storage: DuckDBStorage) -> None:
        ...
        await storage.delete_batch(["rec-0", "rec-2", "rec-4"])
