...
# =============================================================================
# Abstract Method Enforcement (CS-44)
# =============================================================================


class TestRSSAdapterInstantiation:
    """RSSFeedAdapter can be instantiated directly with default httpx fetch."""

    def test_can_instantiate_rss_directly(self):
        """RSSFeedAdapter can be instantiated directly (uses httpx by default)."""
        from feedspine.adapter.rss import RSSFeedAdapter

        adapter = RSSFeedAdapter(url="https://example.com/feed.xml", name="test")
        assert adapter.name == "test"

    def test_concrete_rss_subclass_ok(self):
        """A concrete subclass that overrides _fetch_xml can be instantiated."""
        from feedspine.adapter.rss import RSSFeedAdapter

        class ConcreteRSS(RSSFeedAdapter):
            async def _fetch_xml(self) -> str:
                return "<rss><channel></channel></rss>"

        adapter = ConcreteRSS(url="https://example.com/feed.xml", name="ok")
        assert adapter.name == "ok"


class TestJSONAdapterInstantiation:
    """JSONFeedAdapter can be instantiated directly with default httpx fetch."""

    def test_can_instantiate_json_directly(self):
        """JSONFeedAdapter can be instantiated directly (uses httpx by default)."""
        from feedspine.adapter.json import JSONFeedAdapter

        adapter = JSONFeedAdapter(url="https://api.example.com/items", name="test")
        assert adapter.name == "test"

    def test_concrete_json_subclass_ok(self):
        """A concrete subclass that overrides _fetch_json can be instantiated."""
        from feedspine.adapter.json import JSONFeedAdapter

        class ConcreteJSON(JSONFeedAdapter):
            async def _fetch_json(self):
                return [{"id": "1"}]

        adapter = ConcreteJSON(url="https://api.example.com/items", name="ok")
        assert adapter.name == "ok"
