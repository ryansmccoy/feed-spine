...
class MockFileFeedAdapter(FileFeedAdapter):
    ...
    async def _parse_file(self, content: bytes) -> AsyncIterator[dict[str, Any]]:
        ...
        for _i, line in enumerate(content.decode().strip().split("\n")):
            if line:
                parts = line.split(",")
...
class TestFileSnapshot:
    ...
    def test_snapshot_equality(self) -> None:
        ...
        s1 = FileSnapshot(path="/a", content_hash="hash1", fetched_at=datetime.now(UTC))
        s2 = FileSnapshot(path="/b", content_hash="hash1", fetched_at=datetime.now(UTC))
        s3 = FileSnapshot(path="/a", content_hash="hash2", fetched_at=datetime.now(UTC))
    ...
    def test_has_changed_no_previous(self) -> None:
        ...
        current = FileSnapshot(path="/data", content_hash="new", fetched_at=datetime.now(UTC))
        assert current.has_changed(None) is True

    def test_has_changed_same_hash(self) -> None:
        """Test has_changed with same hash."""
        old = FileSnapshot(path="/data", content_hash="same", fetched_at=datetime.now(UTC))
        new = FileSnapshot(path="/data", content_hash="same", fetched_at=datetime.now(UTC))
        assert new.has_changed(old) is False

    def test_has_changed_different_hash(self) -> None:
        """Test has_changed with different hash."""
        old = FileSnapshot(path="/data", content_hash="old", fetched_at=datetime.now(UTC))
        new = FileSnapshot(path="/data", content_hash="new", fetched_at=datetime.now(UTC))
