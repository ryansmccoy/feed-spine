...
|-----------|---------|----------|
| `core/` | Framework internals | config, exceptions, app factory |
| `models/` | Pydantic data models | Record, Sighting, Layer |
| `protocols/` | Abstract interfaces | StorageBackend, SearchBackend |
| `storage/` | Storage implementations | MemoryStorage, PostgresStorage |
| `search/` | Search implementations | MemorySearch, ElasticsearchSearch |
| `cache/` | Cache implementations | MemoryCache, RedisCache |
| `ops/` | Business logic layer | query, export, schedules |
| `pipeline/` | Processing pipeline | stages, dedup, runner |
| `services/` | Orchestration services | FeedCollectionService |
| `adapter/` | Feed source adapters | RSSFeedAdapter, JSONFeedAdapter |
