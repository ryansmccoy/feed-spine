...
Usage:
    ...
    storage = create_storage(
        "postgresql://user:password@timescale:5432/feedspine",
        use_timescale=True,
        pool_size=20,
    )
...
def _create_memory(_conn: str, _opts: StorageOptions) -> Any:
    from feedspine.storage.memory import MemoryStorage

    return MemoryStorage()


def _create_sqlite(connection_string: str, options: StorageOptions) -> Any:
    from feedspine.storage.backends.sqlite import SQLiteStorage

    db_path = connection_string[10:] if connection_string.startswith("sqlite:///") else connection_string
    if options.data_dir and not os.path.isabs(db_path):
        db_path = os.path.join(options.data_dir, db_path)
    return SQLiteStorage(db_path)


def _create_duckdb(connection_string: str, options: StorageOptions) -> Any:
    from feedspine.storage.backends.duckdb import DuckDBStorage

    db_path = connection_string[10:] if connection_string.startswith("duckdb:///") else connection_string
    if options.data_dir and not os.path.isabs(db_path):
        db_path = os.path.join(options.data_dir, db_path)
    return DuckDBStorage(db_path)


def _create_postgresql(connection_string: str, options: StorageOptions) -> Any:
    ...
    return RepositoryStorageBackend(
        ...
        use_timescale=options.use_timescale,
        auto_migrate=options.auto_migrate,
    )


def _create_timescale(connection_string: str, options: StorageOptions) -> Any:
    from feedspine.storage.repository_backend import RepositoryStorageBackend
...
_STORAGE_REGISTRY: dict[str, Callable[[str, StorageOptions], Any]] = {
    "memory": _create_memory,
    "sqlite": _create_sqlite,
    "duckdb": _create_duckdb,
    "postgresql": _create_postgresql,
    "timescale": _create_timescale,
}


def register_storage_backend(name: str, creator: Callable[[str, StorageOptions], Any]) -> None:
    """Register a custom storage backend creator.

    Args:
        name: Storage type name (used in connection strings and create_storage()).
        creator: Factory function taking (connection_string, options) and returning a storage instance.

    Example:
        >>> from feedspine.storage.factory import register_storage_backend
        >>> def create_redis(conn: str, opts) -> "RedisStorage":
        ...     from my_plugin import RedisStorage
        ...     return RedisStorage(conn)
        >>> register_storage_backend("redis", create_redis)
    """
    _STORAGE_REGISTRY[name] = creator
    ...
    **kwargs: Any,
    ...
    Args:
        connection_string: Database connection string or file path
            ...
            - "postgresql://user:password@host/db" -> SQLAlchemyStorage
    ...
    if connection_string is None:
        connection_string = os.environ.get("FEEDSPINE_DATABASE_URL", os.environ.get("DATABASE_URL", "memory://"))
    ...
    # Create appropriate backend via registry
    creator = _STORAGE_REGISTRY.get(storage_type)
    if creator is None:
        valid_types = ", ".join(sorted(_STORAGE_REGISTRY.keys()))
        raise ValueError(f"Unknown storage type: {storage_type}. Valid types: {valid_types}")

    return creator(connection_string, options)
...
class StorageEnvironment:
    ...
    database_url: str = field(
        default_factory=lambda: os.environ.get("FEEDSPINE_DATABASE_URL", os.environ.get("DATABASE_URL", "memory://"))
    )
    data_dir: str = field(default_factory=lambda: os.environ.get("FEEDSPINE_DATA_DIR", "./data"))
    pool_size: int = field(default_factory=lambda: int(os.environ.get("FEEDSPINE_POOL_SIZE", "5")))
    use_timescale: bool = field(
        default_factory=lambda: os.environ.get("FEEDSPINE_USE_TIMESCALE", "false").lower() == "true"
    )
    batch_size: int = field(default_factory=lambda: int(os.environ.get("FEEDSPINE_BATCH_SIZE", "1000")))
    env: str = field(default_factory=lambda: os.environ.get("FEEDSPINE_ENV", "development"))
...
def get_docker_connection_string(
    ...
    Reads credentials from environment variables ``FEEDSPINE_DB_USER`` and
    ``FEEDSPINE_DB_PASSWORD`` when *user*/*password* are not supplied.
    ...
    Args:
        ...
        user: Database user (default: ``FEEDSPINE_DB_USER`` env var)
        password: Database password (default: ``FEEDSPINE_DB_PASSWORD`` env var)

    Returns:
        PostgreSQL connection string

    Raises:
        ValueError: If user or password cannot be resolved.
    """
    ...
    user = user or os.environ.get("FEEDSPINE_DB_USER")
    password = password or os.environ.get("FEEDSPINE_DB_PASSWORD")
    if not user or not password:
        raise ValueError(
            "Database credentials required. Set FEEDSPINE_DB_USER and "
            "FEEDSPINE_DB_PASSWORD environment variables, or pass user/password explicitly."
        )
...
```python
storage = create_storage(
    "postgresql://$USER:$PASSWORD@localhost:5433/feedspine",
    use_timescale=True,
)
await storage.initialize()
```
...
```python
storage = create_storage(
    "postgresql://$USER:$PASSWORD@localhost:6432/feedspine",
