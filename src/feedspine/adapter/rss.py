...
class RSSFeedAdapter(BaseFeedAdapter):
    ...
    async def _fetch_xml(self) -> str:
        ...
        Default implementation uses httpx. Override to use a different
        HTTP client or to provide mock data in tests.
    ...
    def _is_atom_feed(self, root: ET.Element) -> bool:
        ...
        return root.tag == f"{{{self.ATOM_NS}}}feed" or root.tag == "feed" or "atom" in root.tag.lower()
    ...
    def _to_candidate(self, item: dict[str, Any]) -> RecordCandidate:
        ...
        natural_key = (
            item.get("guid") or item.get("id") or item.get("url") or f"{self.name}:{item.get('title', 'unknown')}"
        )
