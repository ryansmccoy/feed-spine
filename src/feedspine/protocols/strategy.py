...
Defines protocols for intelligent collection strategies that optimize
fetching across multiple sources with different trade-offs (efficiency,
freshness, coverage). E.g., SEC EDGAR: Quarterly > Daily > RSS.
"""
...
@dataclass
class CollectionPlan[SourceT: str]:
...
class BaseCollectionStrategy[SourceT: str](ABC):
