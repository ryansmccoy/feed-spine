...
class EntityEnricher:
    ...
    async def can_enrich(self, record: Record) -> bool:
        ...
        if record.metadata and hasattr(record.metadata, "extra") and record.metadata.extra.get("entity_id"):
            return False
        ...
        return bool(content.get("cik") or content.get("ticker") or content.get("name") or content.get("company_name"))
    ...
    async def enrich(self, record: Record) -> EnrichmentResult:
        ...
        logger.debug(f"Enriched record {record.id} with entity: {entity_info.get('entity_name')}")
