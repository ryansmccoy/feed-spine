...
class CompareResult(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.ticker} {self.period}: {symbol} EPS {self.eps_surprise:+.1%} | Rev {self.revenue_surprise:+.1%}"
            if self.revenue_surprise
            else f"{self.ticker} {self.period}: {symbol} EPS {self.eps_surprise:+.1%}"
        )
...
class CalendarQuery:
    ...
    async def to_dataframe(self):
        ...
        for e in events:
            ...
            if True:  # Always include for demo
...
class CompareAPI:
    ...
    async def __call__(self, ticker: str, period: str = "2026:Q1") -> CompareResult:
        ...
        events = _mock_events()
        event = next(
            (e for e in events if e.ticker.upper() == ticker.upper() and e.status == EventStatus.RELEASED), None
        )
...
async def demo():
    ...
    print("   >>> result = await calendar.check('META')")
    print(f"   {result}")
