...
Run with:
    python examples/05_earnings/04_earnings_rest_api.py
...
class CheckResponse(BaseModel):
    ...
    direction: SurpriseDirection | None = None
    beat: bool = False
    miss: bool = False
    inline: bool = False
    links: EventLinks = EventLinks()
...
def _mock_events() -> list[EarningsEventResponse]:
    ...
    events = [
        ...
        EarningsEventResponse(
            ...
            eps_actual=4.42,
            eps_surprise=-0.018,
            revenue_estimate_mm=42000,
            revenue_actual_mm=41500,
            revenue_surprise=-0.012,
...
async def calendar_today(
    sector: str | None = Query(None, description="Filter by sector"),  # noqa: B008
    status: EventStatus | None = Query(None, description="Filter by status"),  # noqa: B008
):
    """Get today's earnings calendar."""
...
async def compare_earnings(request: CompareRequest):
    ...
    events = _mock_events()
    event = next(
        (e for e in events if e.ticker.upper() == request.ticker.upper() and e.status == EventStatus.RELEASED), None
    )
...
async def recent_releases(hours: int = Query(24, description="Hours to look back")):
    ...
    for e in events:
        writer.writerow(
            [
                ...
                e.eps_actual,
                e.eps_surprise,
                e.revenue_estimate_mm,
                e.revenue_actual_mm,
                e.links.ir,
                e.links.press_release,
            ]
        )
...
if __name__ == "__main__":
    ...
    # Support demo mode for automated testing (no server)
    if os.environ.get("FEEDSPINE_DEMO_MODE") or "--demo" in sys.argv:
        print("=" * 70)
        print("  🌐 EARNINGS REST API DEMO (Demo Mode)")
        print("=" * 70)
        print(__doc__)
        print("\n✓ Demo mode: API structure shown above")
        print("  To run actual server: python examples/05_earnings/04_earnings_rest_api.py")
        sys.exit(0)
