...
def get_mock_event(ticker: str) -> EarningsEvent | None:
...
def format_table(events: list[EarningsEvent], compact: bool = False) -> str:
    ...
    if compact:
        ...
        output.write("┌" + "─" * 88 + "┐\n")
        output.write(
            f"│ {'TIME':<5} {'TICKER':<7} {'COMPANY':<30} {'STATUS':<12} {'EPS EST':<9} {'EPS ACT':<9} {'SURPRISE':<10}│\n"
        )
        output.write("├" + "─" * 88 + "┤\n")
    ...
    for event in sorted(events, key=lambda e: (e.report_time.value, e.ticker)):
        ...
        if compact:
            output.write(
                f"{event.report_time.value:<5} {event.ticker:<6} {status_text:<10} {eps_est:<8} {eps_act:<8} {surprise:<10}\n"
            )
        else:
            company = event.company_name[:28] + ".." if len(event.company_name) > 30 else event.company_name
            output.write(
                f"│ {event.report_time.value:<5} {event.ticker:<7} {company:<30} {status_text:<12} {eps_est:<9} {eps_act:<9} {surprise:<10}│\n"
            )
...
def format_check_result(event: EarningsEvent | None, ticker: str) -> str:
    ...
    if released:
        ...
        for event in released:
            if event.surprise_direction == SurpriseDirection.BEAT:
                print(
                    f"   ✅ {event.ticker} BEAT: ${event.eps_actual:.2f} vs ${event.eps_estimate:.2f} ({event.eps_surprise:+.1%})"
                )
            elif event.surprise_direction == SurpriseDirection.MISS:
                print(
                    f"   ❌ {event.ticker} MISSED: ${event.eps_actual:.2f} vs ${event.eps_estimate:.2f} ({event.eps_surprise:+.1%})"
                )
        print()
