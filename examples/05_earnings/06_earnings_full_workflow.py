...
def _is_demo_mode() -> bool:
    """Check if running in demo mode (fast CI/test execution)."""
...
class CalendarEvent(BaseModel):
    ...
    entity_id: str | None = None  # Link to EntitySpine
    eps_estimate: float | None = None
    eps_actual: float | None = None
    revenue_estimate_mm: float | None = None
    revenue_actual_mm: float | None = None
    ...
    @property
    def surprise_direction(self) -> SurpriseDirection | None:
        s = self.eps_surprise
        if s is None:
            return None
        if s > 0.01:
    ...
    def with_actuals(self, eps: float, revenue_mm: float | None = None) -> CalendarEvent:
...
class ObservationStore:
    ...
    async def get_latest(self, entity_id: str, metric: str) -> Observation | None:
...
class ReportGenerator:
    ...
    async def generate_summary(self, events: list[CalendarEvent]) -> str:
        ...
        for e in released:
            sym = (
                "✅"
                if e.surprise_direction == SurpriseDirection.BEAT
                else "❌"
                if e.surprise_direction == SurpriseDirection.MISS
                else "➡️"
            )
    ...
    async def export_csv(self, events: list[CalendarEvent]) -> str:
        ...
        for e in events:
            lines.append(
                f"{e.ticker},{e.company_name},{e.report_date},{e.report_time.value},{e.status.value},{e.eps_estimate},{e.eps_actual or ''},{e.eps_surprise or ''}"
            )
        return "\n".join(lines)
...
async def full_workflow():
    ...
    # Demo mode: show what would happen and exit early
    if _is_demo_mode():
        print("=" * 70)
        print("  🚀 FULL EARNINGS WORKFLOW DEMO (Demo Mode)")
        print("=" * 70)
        print("""
In full mode, this demo would run through:

    📌 STEP 1: LOAD CALENDAR
       • Fetch earnings calendar for today
       • Merge from multiple sources (Polygon, Yahoo, etc.)

    📌 STEP 2: RESOLVE ENTITIES
       • Match tickers to EntitySpine IDs
       • Link company data across sources

    📌 STEP 3: ENRICH WITH ESTIMATES
       • Load consensus estimates from vendors
       • Track analyst count, high/low range

    📌 STEP 4: STORE OBSERVATIONS
       • Save to FeedSpine observation store
       • Version with as_of timestamps

    📌 STEP 5: WATCH FOR RELEASES
       • Monitor for earnings announcements
       • Real-time event detection

    📌 STEP 6: COMPARE & ALERT
       • Calculate beat/miss/inline
       • Send alerts for watchlist

    📌 STEP 7: GENERATE REPORTS
       • Summary table, charts, export

Run without FEEDSPINE_DEMO_MODE=1 for the full simulation.
""")
        return
    ...
    for event in events:
        ...
        resolved_events.append(CalendarEvent(**{**event.model_dump(), "entity_id": entity_id}))
    ...
    async for released_event in monitor.watch(events):
        ...
        all_events = [released_event if e.ticker == released_event.ticker else e for e in all_events]
