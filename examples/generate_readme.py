...
def generate_readme(
    ...
    w()
    w(f"> **{total} examples** across **{len(cats)} categories** — auto-generated from docstrings.")
    w("> Regenerate: `python examples/generate_readme.py`")
    w()
    w("---")
    w()
    ...
    w("## Learning Path")
    w()
    w("Categories are numbered by conceptual dependency — start at `01` and work forward.")
    w()
    ...
    if arch_examples:
        ...
        w()
        w("These examples include architecture diagrams — key for understanding data flow and component interaction.")
        w()
    ...
    w("---")
    w()
    w(f"*Generated on {datetime.now(UTC).strftime('%Y-%m-%d')} from {total} examples across {len(cats)} categories.*")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Auto-generate examples/README.md from ExampleRegistry.")
    parser.add_argument(
        "--check",
        action="store_true",
